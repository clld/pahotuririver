...
from clldutils.misc import nfilter, data_url
...
def main(args):
    ...
    data.add(
        common.Dataset,
        pahotuririver.__name__,
        id=pahotuririver.__name__,
        domain='',
    ...
    for form in args.cldf.iter_rows('FormTable', 'id', 'form', 'languageReference', 'parameterReference', 'source'):
        ...
        audio = None
        if f2a.get(form['id']):
            audio = data_url(args.cldf.directory / anyURI.to_string(f2a[form['id']]), 'audio/mpeg')
        data.add(
            Counterpart,
            form['id'],
            id=form['id'],
